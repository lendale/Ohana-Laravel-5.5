...
var extRef = firebase.database().ref().child('album_extend');
...
firebase.auth().onAuthStateChanged(function(user){
    if(user){
        currentUser = user;
        uid = currentUser.uid;
        console.log('Welcome to Extended Album, ',currentUser.displayName)
    }
    else {
...
$( document ).ready(function() {
    console.log( "ready!" );
    getUserExtId(currentUser.uid)
    getUserClanId(currentUser.uid)
    // retrieveAlbumsFromExtended()
});

$('#album_cover').change(handleExtAlbumInStr);
$('#album_photo').change(handleExtPhotoInStr);
...
function getUserExtId(uid)
{
    firebase
        ...
        .then(function(snapshot) {
            ...
            retrieveAlbumsFromExtended(extendedId)
        })
}

function getExtendedAlbumList(){
    var albumDataRef = firebase.database().ref().child('album_extend').child(extId);
...
function retrieveAlbumsFromExtended() {

    var extDataRef = firebase.database().ref().child('album_extend').child(extendedId);

    console.log(extendedId)

    extDataRef.once('value')
        .then(function(data){
            data.forEach(function(childData){
                extData.push(childData.val())
            })
...
function retrieveExtAlbumPhotos(clicked_key){
    selectedData = extData[clicked_key];

    console.log(selectedData.album_key)
    console.log(selectedData.album_name)

    var selectedDataRef = firebase.database().ref().child('album_extend').child(extendedId).child(selectedData.album_key).child('album_photos');
...
function handleExtPhotoInStr(eventData){
        ...
        task.on('state_changed',
            function complete(){
                ...
                swal({
                    ...
                    function(dismiss) {
                        if (dismiss === "timer") {
                        storeExtPhotoInDb(photoURL,photoKey);
...
function storeExtPhotoInDb(photoURL, photoKey){
...
function handleExtAlbumInStr(eventData){
    var file = eventData.target.files[0];
    var fileName = file.name;
    var fileExtension = fileName.split(".").pop();
        ...
        task.on('state_changed',
            ...
            function complete(){
                ...
                swal({
                  ...
                  function(dismiss) {
                      if (dismiss === "timer") {
                        storeExtAlbumInDb(albumCoverURL,albumKey);
                        console.log("kalusot sad sa function");
                        // window.location.href = "/genealogy";
                      }
                  });
...
function storeExtAlbumInDb(albumCoverURL, albumKey){
...
function deleteAlbum(clicked_key){
    ...
    var delete_selected = extData[clicked_key];
    ...
    swal({
        ...
        function deleteInStr(){
            ...
            .then(function(){
                ...
                .then(function(){
                    extAlbumRef.child(delete_selected.album_key).remove()
                .then(function(){
...
function updateAlbum(clicked_key){
    ...
    var clicked_album = extData[clicked_key];
