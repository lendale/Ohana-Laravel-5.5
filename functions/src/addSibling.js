    ...
    const pro1 = root.child(`users/${pushKey}`).once("value").then(snap => {
        if(snap.exists()) {
                ...
                snap2.forEach(snap3 => {
                    if(snap3.val() !== pushKey) {
                        ...
                        root.child(`users/${snap3.val()}`).once("value").then(snap4 => {
                            ...
                            root.child(`user_family/${uid}/sisters/${pushKey}`).once('value').then(snap5 => {
                                root.child(`immediate_family/${snap5.val().familyId}/${siblingType}/${snap3.val()}`).set(snap3.val())
                                root.child(`extended_family/${snap5.val().extendedId}/${snap3.val()}`).set(snap3.val())
                            })
                        })
                    }
                })
            ...
            root.child(`users/${uid}`).once('value').then(snap2 => {
                ...
                if(snap2.val().gender === "female") siblingType = "sister";
                else siblingType = "brother";
                ...
                root.child(`immediate_family/${snap2.val().familyId}/sister/${pushKey}`).set(pushKey)
                root.child(`extended_family/${snap2.val().extendedId}/${pushKey}`).set(pushKey)

                root.child(`users/${mother}`).once('value').then(snap3 => {
                    root.child(`immediate_family/${snap3.val().familyId}/daughter/${pushKey}`).set(pushKey)
                ...
                root.child(`user_family/${uid}/sisters/${pushKey}`).once('value').then(snap3 => {    
                    root.child(`immediate_family/${snap3.val().familyId}/${siblingType}/${uid}`).set(uid)
                    root.child(`immediate_family/${snap3.val().familyId}/mother`).set(mother)
                    root.child(`immediate_family/${snap3.val().familyId}/father`).set(father)
                    
                    root.child(`extended_family/${snap3.val().extendedId}/${uid}`).set(uid)
                    root.child(`extended_family/${snap3.val().extendedId}/${mother}`).set(mother)
                    root.child(`extended_family/${snap3.val().extendedId}/${father}`).set(father)
                })
            })
    ...
    const pro1 = root.child(`users/${pushKey}`).once("value").then(snap => {
        if(snap.exists()) {
                ...
                snap2.forEach(snap3 => {
                    if(snap3.val() !== pushKey) {
                        ...
                        root.child(`users/${snap3.val()}`).once("value").then(snap4 => {
                            ...
                            root.child(`user_family/${uid}/brothers/${pushKey}`).once('value').then(snap5 => {
                                root.child(`immediate_family/${snap5.val().familyId}/${siblingType}/${snap3.val()}`).set(snap3.val())
                                root.child(`extended_family/${snap5.val().extendedId}/${snap3.val()}`).set(snap3.val())
                            })
                        })
            ...
            root.child(`users/${uid}`).once('value').then(snap2 => {
                ...
                root.child(`immediate_family/${snap2.val().familyId}/brother/${pushKey}`).set(pushKey)
                root.child(`extended_family/${snap2.val().extendedId}/${pushKey}`).set(pushKey)

                root.child(`users/${mother}`).once('value').then(snap3 => {
                    root.child(`immediate_family/${snap3.val().familyId}/son/${pushKey}`).set(pushKey)
                    root.child(`extended_family/${snap3.val().extendedId}/${pushKey}`).set(pushKey)
                })
                ...
                root.child(`user_family/${uid}/brothers/${pushKey}`).once('value').then(snap3 => {    
                    root.child(`immediate_family/${snap3.val().familyId}/${siblingType}/${uid}`).set(uid)
                    root.child(`immediate_family/${snap3.val().familyId}/mother`).set(mother)
                    root.child(`immediate_family/${snap3.val().familyId}/father`).set(father)
