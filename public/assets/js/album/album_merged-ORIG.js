...
function getUserData(uid){
    usersRoot.child(currentUser.uid).once("value")
        .then(function(snapshot) {
            ...
            // check(extendedId)
            // retrieveFromExtendedFam(extendedId)
            // retrieveAlbumsFromUsers(familyId)
...
function deleteAlbum(clicked_key){
        ...
        function deleteInStr(){
            albumRef.child(delete_selected.album_key).remove()
                .then(function(){
                    ...
                    var photoStr = firebase.storage().ref(delete_selected.photo_url)
                    // console.log(delete_selected.photo_url)
                    albumStr.delete()
                    photoStr.delete()
    ...
    function deletePhotos(){
        albumRef.child(delete_selected.album_key).child('album_photos').once('value')
        .then(function (snapPhotoKeys){
            console.log(snapPhotoKeys)
        })
    }
}
