...
function responseEvent(response, albumKey) {
    ...
    if (response == "Going") {
        ...
        firebaseRef.child("event_response").child(albumKey).child(currentUser).remove();
    }
...
function extendedDeleteClick(clicked_key) {
console.log(clicked_key)
    ...
    swal({
        ...
        if (isConfirm) {
            ...
            firebaseRef.child('events_privacy').child('extended').child(currentUser).child(cKey).remove();
...
function extendedUpdateClick(clicked_key2) {
