...
function initFamilyTree(data) {
    var $ = go.GraphObject.make;
    myDiagram = $(go.Diagram, "family_diagram", {
    ...
    myDiagram.nodeTemplateMap.add(
        ...
        $(
            go.Node,
            "Vertical", { locationSpot: go.Spot.Center, locationObjectName: "ICON" },
            $(
                go.Panel, { name: "ICON" },
                $(go.Shape, "Square", {
                    width: 40,
    ...
    myDiagram.addDiagramListener("ObjectSingleClicked", function(e) {
        ...
        console.log("natuplok");
        console.log(!(part instanceof go.Link));
        if (!(part instanceof go.Link)) {
            showModal(part.data);
        }
    });

    setupDiagram(myDiagram, data, 19);
}
...
/**
 * Display the information of a person. The data are coming from the JSON file as an attribute.
 *
 * @param data - The data of a person.
 */
function showModal(data) {
    $("#view_node").modal("open");
    console.log(data);
    $(".modal h3.modal-header-name")
        .empty()
        .append(data.n);
    $(".modal h3.media-heading")
        .empty()
        .append(data.n);
}